...
void runalgorithmB(double r,int step) {
  ...
  do {
    ...
    if((instancelength(last) < lastTimePoint) && (actives < paramM) && (firstactive == -1 || threshold > 0.5)) {
      ...
      //  + estimateinstancesize(instancelength(last+1),nOfActions*3,nOfActions)
      if(memoryused() < flagMemoryLimit) {
	startlength(last+1,instancelength(last+1));
	last += 1;
	actives += 1; // printf("ADD: actives = %i %i\n",actives,paramM);
      } else {
	//	printf("Horizon %i will not be started: memory allocated %.2f MB limit %.2f MB\n",instancelength(last+1),memoryused(),flagMemoryLimit);
      }
    }
...
void runalgorithmC() {
  ...
  do {
    ...
    if(actives < paramM && Clength(last+1) <= lastTimePoint && estimateinstancesize(Clength(last+1),varsPerTime,nOfActions) + memoryused() < flagMemoryLimit) {
