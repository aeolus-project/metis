...
type component_name = Json_zephyrous_output_t.component_name

type package_name = Json_zephyrous_output_t.package_name

type repository_name = Json_zephyrous_output_t.repository_name
...
type require_arity = Json_zephyrous_output_t.require_arity

type resource_consumption = Json_zephyrous_output_t.resource_consumption
...
type component_types = Json_zephyrous_output_t.component_types

type package = Json_zephyrous_output_t.package = {
...
type packages = Json_zephyrous_output_t.packages

type repository = Json_zephyrous_output_t.repository = {
...
type resources_provided = Json_zephyrous_output_t.resources_provided

type location_cost = Json_zephyrous_output_t.location_cost

type location = Json_zephyrous_output_t.location = {
...
type configuration = Json_zephyrous_output_t.configuration = {
...
let read_component = (
  fun p lb ->
    ...
    try
      ...
      assert false;
    with Yojson.End_of_object -> (
        if !bits0 <> 0xf then Ag_oj_run.missing_fields [| !bits0 |] [| "name"; "component_type_workaround"; "location"; "state" |];
        Ag_oj_run.identity x
      )
)
